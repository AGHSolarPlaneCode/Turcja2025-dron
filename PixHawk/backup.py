...
class PixHawkService:
    ...
    def __init__(self, device: str = "/dev/ttyAMA0", baud: int = 57600):
        ...
        if self.enable_gps_data():
            print("GPS messages enabled")
        else:
            print("Failed to enable GPS messages")
    ...
    def enable_gps_data(self) -> bool:
        """
        Sends request to enable GPS messages

        :return: True if operation successful, False if failed
        """
